<x-layout>
    <section class="bg-white rounded shadow p-8 mb-8">
        ...
        <form action="{{ route('crear-hilo-action') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="categoria" id="categoria"
                    ...
                    @foreach ($categorias as $categoria_valor)
                        <option value="{{ $categoria_valor->id }}">{{ $categoria_valor->titulo }}</option>
                    @endforeach
            ...
            <!-- Título del Hilo -->
            <div>
                <label for="titulo" class="block text-lg font-semibold text-gray-700">Título del hilo
                    (pregunta)</label>
                <p class="text-sm text-gray-500 mb-2">¡Haz que los calvos se rasquen la cabeza (si tienen algo de pelo)
                    con tu pregunta!</p>
                <input type="text" name="titulo" id="titulo"
                    class="w-full border border-gray-300 rounded p-3 focus:outline-none focus:ring focus:border-blue-300"
                    placeholder="¿Qué quieres preguntar o compartir?" required>
            </div>

            <!-- Primera Respuesta -->
            <div>
                <label for="contenido" class="block text-lg font-semibold text-gray-700">Tu primer aporte al
                    hilo</label>
            ...
            <div class="text-center">
                <button type="submit"
                    class="bg-blue-600 text-white px-6 py-3 rounded font-bold hover:bg-blue-700 transition">Crear
                    Hilo</button>
            </div>
        </form>
    </section>
